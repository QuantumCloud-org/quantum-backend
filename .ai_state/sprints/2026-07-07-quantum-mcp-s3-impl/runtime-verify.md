...
| 场景 | 类型 | 命令 | 实际输出 | 判定 |
|---|---|---|---|---|
| OAuth / MCP module regression | Maven | `mvn -pl quantum-mcp -am test` | `BUILD SUCCESS`; `quantum-mcp` 10 tests PASS before JSON-RPC addition, 12 tests PASS after final server aggregate | PASS |
| Server aggregate compile/test | Maven | `mvn -pl quantum-server -am test` | Reactor 11 modules `BUILD SUCCESS`; `quantum-mcp` 12 tests, dependency modules 66 tests | PASS |
...
- 代码级验收已覆盖: PKCE RFC 向量、授权码一次性、resource mismatch、refresh reuse revoke、Bearer filter 401 metadata、UserContext/permission fail-closed、Manifest metadata、JSON-RPC wrapper。
